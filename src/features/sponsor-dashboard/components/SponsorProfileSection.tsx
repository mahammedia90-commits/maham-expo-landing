...
import { sponsorAuthService } from '@/features/auth/services/sponsorAuthService';
import { ChangePasswordModal } from '@/features/auth/components/ChangePasswordModal';
import { EmailVerificationForm } from '@/features/auth/components/EmailVerificationForm';
import type { SponsorLevel } from '@/shared/types';

export function SponsorProfileSection() {
  const { t, isRtl } = useLanguageStore();
  const { user, setAuth } = useSponsorAuthStore();
  ...
  const updateProfile = useMutation({
    mutationFn: (data: typeof form) => sponsorAuthService.updateProfile(data),
    onSuccess: (response) => {
      if (response.data && user) {
        setAuth({ ...user, ...response.data }, useSponsorAuthStore.getState().token!);
      }
      setIsEditing(false);
      setSuccessMsg(t.sponsorDashboard.profileUpdated);
  ...
  const levelLabels: Record<SponsorLevel, string> = {
    platinum: t.sponsorDashboard.platinum,
    gold: t.sponsorDashboard.gold,
    silver: t.sponsorDashboard.silver,
    official_partner: t.sponsorDashboard.official_partner,
    media_partner: t.sponsorDashboard.media_partner,
    strategic_alliance: t.sponsorDashboard.strategic_alliance,
  };
  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      >
        {t.sponsorDashboard.profileTitle}
      </motion.h1>
      ...
      >
            ...
            <div>
              ...
              {user?.sponsorLevel && (
                <span className="inline-block mt-1 text-xs px-3 py-0.5 rounded-full bg-white/20 text-white font-medium">
                  {levelLabels[user.sponsorLevel]}
                </span>
              )}
              {user?.createdAt && (
                <p className="text-white/60 text-xs mt-1">
                  {t.sponsorDashboard.memberSince} {new Date(user.createdAt).toLocaleDateString()}
                </p>
              )}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {[
            { key: 'name' as const, label: t.sponsorAuth.name, type: 'text' },
            { key: 'email' as const, label: t.sponsorAuth.email, type: 'email' },
            { key: 'phone' as const, label: t.sponsorAuth.phone, type: 'tel' },
            { key: 'companyName' as const, label: t.sponsorAuth.companyName, type: 'text' },
          ].map(({ key, label, type }) => (
            <div key={key}>
          ...
          <div className="flex gap-3 pt-2">
            {isEditing ? (
              <>
                ...
                >
                  {updateProfile.isPending ? t.sponsorDashboard.saving : t.sponsorDashboard.saveChanges}
                </motion.button>
                <button
                  type="button"
                  onClick={() => {
              ...
              >
                {t.sponsorDashboard.editProfile}
              </motion.button>
            )}
          </div>
        </form>
      ...
      >
        ...
        <div className="flex items-center justify-between py-3">
          <div>
            <p className="text-sm font-medium text-gray-900 dark:text-white">{t.merchantAuth.changePasswordTitle}</p>
