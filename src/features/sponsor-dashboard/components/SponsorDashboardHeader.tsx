...
export function SponsorDashboardHeader({ onMenuToggle }: SponsorDashboardHeaderProps) {
  const { t, toggleLanguage, language } = useLanguageStore();
  const { toggleTheme, theme } = useThemeStore();
  const { user } = useSponsorAuthStore();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <p className="text-sm text-gray-500 dark:text-gray-400">{t.sponsorDashboard.welcome}</p>
