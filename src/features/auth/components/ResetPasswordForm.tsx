...
export function ResetPasswordForm() {
  ...
  const resetPassword = useMutation({
    mutationFn: () => authService.resetPassword(email, tokenFromUrl, password, confirmPassword),
  });

  const validate = () => {
    const newErrors: typeof errors = {};
    if (!email.trim()) newErrors.email = t.merchantAuth.emailRequired;
    if (!password.trim()) newErrors.password = t.merchantAuth.newPasswordRequired;
    else if (password.length < 8) newErrors.password = t.merchantAuth.newPasswordMin;
    if (password !== confirmPassword) newErrors.confirmPassword = t.merchantAuth.passwordMismatch;
    setErrors(newErrors);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {t.merchantAuth.resetPasswordTitle}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            {t.merchantAuth.resetPasswordSubtitle}
          </p>
        </div>
        ...
        {resetPassword.isSuccess && (
          ...
          >
            {t.merchantAuth.resetPasswordSuccess}
          </motion.div>
        )}
        ...
        {resetPassword.isError && (
          ...
          >
            {(resetPassword.error as { response?: { data?: { message?: string } } })?.response?.data?.message || t.merchantAuth.resetPasswordTitle}
          </motion.div>
        )}
        ...
        {!resetPassword.isSuccess ? (
          <>
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-xl border ${errors.confirmPassword ? 'border-red-400' : 'border-gray-300 dark:border-gray-600'} bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#987012] focus:border-transparent outline-none transition-all`}
                  dir="ltr"
                  autoComplete="new-password"
                />
            ...
            >
              {resetPassword.isPending ? t.merchantAuth.resettingPassword : t.merchantAuth.resetPassword}
            </motion.button>
          </>
        ) : (
          <div className="text-center">
            <Link
              href={ROUTES.LOGIN}
              className="inline-flex items-center gap-2 text-[#987012] hover:text-[#B8860B] dark:text-[#D4B85A] dark:hover:text-[#E8C860] transition-colors font-medium text-sm"
            >
