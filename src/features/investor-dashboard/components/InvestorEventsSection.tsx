...
export function InvestorEventsSection() {
  ...
  return (
    <div className="min-h-screen" dir="rtl">
      <div className="mb-5 sm:mb-7"><h1 className="text-3xl sm:text-4xl lg:text-[3.25rem] font-black text-gray-900 dark:text-white text-right leading-tight">{en ? 'Investment Opportunities ' : 'فرص الفعاليات '}<span className="text-[#987012]">{list.length}</span></h1></div>
      ...
      {sel && <EventDetailModal event={sel} onClose={() => setSel(null)} variant="investor" />}
    </div>
  );
}
