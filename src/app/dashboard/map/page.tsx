...
export default function ExpoMap() {
  ...
  const statusLabels: Record<BoothStatus, string> = {
    ...
    reserved: isAr ? "محجوز مبدئياً" : "Reserved",
  ...
  const getBoothName = (b: Booth) => isAr ? b.nameAr : b.nameEn;

  const handleBook = (booth: Booth) => {
    if (booth.status !== "available") {
      toast.error(isAr ? "هذه الوحدة غير متاحة حالياً" : "This unit is not available");
      return;
    }
    toast.success(`${isAr ? "تمت الإضافة للسلة" : "Added to cart"} — ${getBoothName(booth)}`);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder={isAr ? "ابحث عن وحدة..." : "Search for a unit..."}
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 glass-card rounded-2xl overflow-hidden relative" style={{ minHeight: "300px" }}>
          <img src={MAP_BG} alt={isAr ? "خريطة المعرض" : "Expo Map"} className="w-full h-full object-cover absolute inset-0 opacity-60" />
          ...
          {selected ? (
            ...
            >
              ...
              <div className="space-y-3 mb-6">
                {[
                  { label: isAr ? "رقم الوحدة" : "Unit ID", value: selected.id },
                  { label: isAr ? "المنطقة" : "Zone", value: `${isAr ? "المنطقة" : "Zone"} ${selected.zone}` },
                  { label: isAr ? "المساحة" : "Size", value: selected.size },
                  { label: isAr ? "السعر" : "Price", value: `${selected.price} ${isAr ? "ر.س" : "SAR"}` },
                ].map((d, i) => (
