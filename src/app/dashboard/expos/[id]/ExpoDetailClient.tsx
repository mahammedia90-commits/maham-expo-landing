...
import { useTraderStore, generateId } from "@/features/merchant-dashboard/store/useTraderStore";
...
// Main Page Component
// ═══════════════════════════════════════════════════════════════════
export default function ExpoDetailClient() {
  ...
  const handleContractAccepted = () => {
    if (!holdBooth || !selectedBooth) return;
    // Generate a booking ID and save immediately as pending_review
    const newBookingId = generateId("BK");
    setPendingBookingId(newBookingId);

    const { addBooking, addNotification } = useTraderStore.getState();
    ...
    addBooking({
      ...
      unitAr: `${selectedBooth.code} — ${selectedBooth.type}`,
      unitEn: `${selectedBooth.code} — ${selectedBooth.type}`,
      zone: selectedBooth.zone,
      boothType: selectedBooth.type,
      boothSize: selectedBooth.size,
      price: selectedBooth.price,
      deposit: selectedBooth.price * 0.05,
      paidAmount: selectedBooth.price * 0.05,
      remainingAmount: selectedBooth.price * 0.95,
      status: "pending_review",
      paymentStatus: "deposit_paid",
      contractGenerated: true,
      contractId: generateId("CNT"),
      createdAt: new Date().toISOString(),
      services: selectedBooth.featureKeys,
      location: `${expo.venue} — ${selectedBooth.zone}`,
    });
    ...
    addNotification({
      ...
      message: isAr
        ? `تم إرسال طلب حجز الجناح ${selectedBooth.code} للمراجعة`
        : `Booth ${selectedBooth.code} booking request sent for review`,
      read: false,
      createdAt: new Date().toISOString(),
      ...
      setReviewTimer(prev => {
        ...
        if (next >= 10 + Math.floor(Math.random() * 5)) {
          ...
          if (approved) {
            ...
            if (pendingBookingId) {
              useTraderStore.getState().updateBookingStatus(pendingBookingId, "approved");
            }
            ...
            // Update booking status in store
            if (pendingBookingId) {
              useTraderStore.getState().updateBookingStatus(pendingBookingId, "rejected");
            }
  ...
  const handleConfirmPayment = () => {
    ...
    // Update existing booking to confirmed + add payment
    const { updateBookingStatus, updateBookingPayment, addPayment, addNotification } = useTraderStore.getState();

    updateBookingStatus(pendingBookingId, "confirmed");
    updateBookingPayment(pendingBookingId, selectedBooth.price * 0.95, "fully_paid");
    ...
    addPayment({
      ...
      descAr: `دفع كامل — ${selectedBooth.code}`,
      descEn: `Full payment — ${selectedBooth.code}`,
    });
    ...
    addNotification({
      ...
      message: isAr
        ? `تم تأكيد حجز الجناح ${selectedBooth.code} في ${expo.nameAr}`
        : `Booth ${selectedBooth.code} confirmed at ${expo.nameEn}`,
      read: false,
      createdAt: new Date().toISOString(),
      link: "/dashboard/bookings",
    });
